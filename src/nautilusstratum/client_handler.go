...
func (c *clientListener) NewBlockAvailable(kapi *NautilusApi) {
	c.clientLock.Lock()
		...
		go func(client *gostratum.StratumContext) {
			...
			if err != nil {
				if strings.Contains(err.Error(), "Could not decode address") {
					...
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from nautilus, malformed address: %s", err))
					client.Disconnect() // unrecoverable
				} else {
					RecordWorkerError(client.WalletAddr, ErrFailedBlockFetch)
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from nautilus: %s", err))
				}
				return
			}
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := kapi.nautiliad.GetBalancesByAddresses(addresses)
				if err != nil {
					c.logger.Warn("failed to get balances from nautilus, prom stats will be out of date", zap.Error(err))
					return
				}
