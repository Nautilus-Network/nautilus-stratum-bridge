...
var walletRegex = regexp.MustCompile("nautilus:[a-z0-9]+")
...
func CleanWallet(in string) (string, error) {
	...
	if !strings.HasPrefix(in, "nautilus:") {
		return CleanWallet("nautilus:" + in)
	}

	// has nautilus: prefix but other weirdness somewhere
	if walletRegex.MatchString(in) {
		return in[0:67], nil
	}
	return "", errors.New("unable to coerce wallet to valid nautilus address")
}
