...
import (
	...
	"github.com/GRinvestPOOL/nautilus-stratum-bridge/src/nautilusstratum"
	"gopkg.in/yaml.v2"
)
...
func main() {
	...
	cfg := nautilusstratum.BridgeConfig{}
	...
	flag.StringVar(&cfg.RPCServer, "nautilus", cfg.RPCServer, "address of the nautiliad node, default `localhost:42110`")
	...
	if cfg.BlockWaitTime == 0 {
		cfg.BlockWaitTime = 5 * time.Second // this should never happen due to ntl 1s block times
	}

	log.Println("----------------------------------")
	log.Printf("initializing bridge")
	log.Printf("\tnautiliad:        %s", cfg.RPCServer)
	...
	if err := nautilusstratum.ListenAndServe(cfg); err != nil {
		log.Println(err)
	}
}
